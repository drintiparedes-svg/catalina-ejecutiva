...
| `qa-captura` | El micrófono aislado: que el worklet entregue exactamente lo que se dijo, en lotes de 100 ms transferidos y en orden, y que el base64 rápido sea idéntico al que sustituye. |
...
| `qa-voz` | La sesión de ElevenLabs de verdad con un WebSocket doble: el micrófono (un tono de 440 Hz) recorre worklet → PCM → base64 → socket y llega a 16 kHz con la frecuencia intacta; el audio del agente entra al reproductor; pausar corta el envío; una interrupción vacía la cola. |
